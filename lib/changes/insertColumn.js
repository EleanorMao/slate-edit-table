...
import { TablePosition, createCell, getTableInfo } from '../utils';
import moveSelection from './moveSelection';

import type Options from '../options';
import getCellsAtRow from '../utils/getCellsAtRow';
...
function insertColumn(
    ...
    const cellIndex = pos.getColumnIndex();
    ...
    preColInfo.forEach((cellInfo, realRowIndex) => {
        if (cellInfo.colSpan > 1 && cellInfo.colIndex + cellInfo.colSpan > at) {
            const theCell = getCellsAtRow(opts, table, cellInfo.rowIndex).get(
                cellInfo.cellIndex
            );
